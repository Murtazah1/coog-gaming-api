...
async def get_user(user_id: int, db: db_dependency):
    ...
    if user is None:
        HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User was not found")
    return user

# getting multiple users
@app.get("/users/", status_code=status.HTTP_200_OK)
# skip is how many inital rows will be skipped in the query, so if 10 it will skip first 10 rows
